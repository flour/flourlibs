...
var projectDir = project == "All" ? "./" : $"./src/{project}";
...
Task("Test")
    ...
    .DoesForEach(GetFiles("./tests/" + (project == "All" ? "" : $"{project}.Tests/") + "**/*.csproj"), prj =>
    {
        DotNetCoreTest(
            ...
            {
                Configuration = configuration,
                Logger = $"trx;LogFileName={prj.GetFilenameWithoutExtension()}.trx",
                NoBuild = true,
                NoRestore = true,
                ResultsDirectory = artefactsDirectory,
                ArgumentCustomization = x => x
                    .Append("--blame")
                    .AppendSwitch("--logger", $"html;LogFileName={prj.GetFilenameWithoutExtension()}.html")
